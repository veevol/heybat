-- Matching board: counts + paginated page keys filtered/searched in Postgres.
...
BEGIN
  ...
  SELECT
    COUNT(DISTINCT CASE
      WHEN status = 'terverifikasi' AND kode_obat_yelo IS NOT NULL THEN kode_obat_yelo
    END)::int,
    COUNT(*) FILTER (WHERE status = 'menunggu_verifikasi')::int,
    COUNT(*) FILTER (
      WHERE status IS NULL
         OR (status = 'ditolak' AND kode_obat_yelo IS NOT NULL)
         OR status = 'usulan'
    )::int,
    COUNT(*) FILTER (WHERE status = 'ditolak' AND kode_obat_yelo IS NULL)::int
...
DECLARE
  ...
  v_items jsonb := '[]'::jsonb;
BEGIN
  -- Normalize aliases from frontend/docs
  ...
  action_rows AS (
    SELECT
      ...
      CASE
        ...
        WHEN am.status = 'ditolak' AND am.kode_obat_yelo IS NULL THEN 'rejected'
        ELSE 'unmatched'
      END AS kind
    FROM latest_pl pl
...
GRANT EXECUTE ON FUNCTION public.matching_board_counts(uuid, timestamptz) TO service_role;
GRANT EXECUTE ON FUNCTION public.matching_board_counts(uuid, timestamptz) TO authenticated;
GRANT EXECUTE ON FUNCTION public.matching_board_page(uuid, text, text, int, int, timestamptz) TO service_role;
GRANT EXECUTE ON FUNCTION public.matching_board_page(uuid, text, text, int, int, timestamptz) TO authenticated;

COMMENT ON FUNCTION public.matching_board_counts IS
  'Lightweight pill counters for matching board (match/menunggu/belum/no_match).';
COMMENT ON FUNCTION public.matching_board_page IS
  'Paginated matching board keys filtered/searched in DB.';
