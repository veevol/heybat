-- Split board: no_match (staf No Data) vs ditolak (owner tolak, punya kode Yelo).
...
BEGIN
  ...
  INTO v_match, v_menunggu, v_belum, v_no_match, v_ditolak
  FROM joined;
  ...
  action_rows AS (
    SELECT
      ...
      CASE
        ...
        WHEN am.status = 'ditolak' AND am.kode_obat_yelo IS NOT NULL THEN 'ditolak'
        ELSE 'unmatched'
      END AS kind
    FROM latest_pl pl
  ...
  filtered_actions AS (
    ...
    WHERE
      CASE v_status
        ...
        WHEN 'ditolak' THEN kind = 'ditolak'
        WHEN 'all' THEN true
        ELSE false
      END
  ),
...
COMMENT ON FUNCTION public.matching_board_counts IS
  'Board counts: no_match=staf No Data; ditolak=owner tolak (punya Yelo).';
COMMENT ON FUNCTION public.matching_board_page IS
  'Paginated board; kind rejected=no_match, ditolak=owner reject.';
