-- Owner tolak verifikasi (ditolak + kode Yelo) masuk bucket board no_match,
-- sama dengan staf "Tidak Ada yang Cocok". Blm Diajukan hanya tanpa matching aktif.
...
COMMENT ON FUNCTION public.matching_board_counts IS
  'Board counts: no_match = semua status ditolak (staf no-data + owner tolak).';
COMMENT ON FUNCTION public.matching_board_page IS
  'Paginated matching board; ditolak (dengan/tanpa Yelo) → kind rejected / filter no_match.';
